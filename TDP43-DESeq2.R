# Consider the README file for more information
...
thresh <- 100
countdata <- countdata[rowSums(countdata) > thresh, ]   ##fix: countdata has no ID column left, [,-1] was dropping rep1 from the filter

condition <- factor(c("H","H","H","D","D","D"), levels = c("H","D"))   ##fix: explicit levels, H as reference (was defaulting to alphabetical, D as reference)
coldata <- data.frame(sampleName, condition, row.names = sampleName)   ##fix: rownames(coldata) must match colnames(countdata) for DESeqDataSetFromMatrix
